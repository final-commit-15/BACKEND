...
class AuthService:
    ...
    async def register(db: AsyncSession, user_data: UserCreate) -> User:
        try:
            ...
            import logging
            logging.getLogger(__name__).error(f"Register DB error: {type(e).__name__}: {e}")
    ...
    async def login(db: AsyncSession, email: str, password: str, ip_address: str = None) -> TokenResponse:
        try:
            ...
            import logging
            logging.getLogger(__name__).error(f"Login DB error: {type(e).__name__}: {e}")
            raise DatabaseError("Database temporarily unavailable") from e
    ...
    async def refresh_token(db: AsyncSession, refresh_token: str, ip_address: str = None, rotate: bool = None) -> TokenResponse:
        try:
            ...
            import logging
            logging.getLogger(__name__).error(f"Refresh token DB error: {type(e).__name__}: {e}")
            raise DatabaseError("Database temporarily unavailable") from e
    ...
    async def logout(db: AsyncSession, user_id: str, access_token: str = None, refresh_token: str = None, revoke_all: bool = False, ip_address: str = None) -> bool:
        ...
        try:
            ...
            import logging
            logging.getLogger(__name__).error(f"Logout DB error: {type(e).__name__}: {e}")
            raise DatabaseError("Database temporarily unavailable") from e